...
class general_model extends CI_Model{
	...
	function deleteSectorDepartmentByCategory($categoryID){
		 $this->db->delete($this->table_sector_department,array("category_id"=>$categoryID));
	}
	...
	function deleteAllChildCategory($id){
		 $this->db->delete($this->table_category,array("parent"=>$id));
		 return $this->db->affected_rows();
	}
	...
	function updateBatchSubCategory($dataArray,$element){
		$this->db->update_batch($this->table_category, $dataArray, $element); 
	}
	...
	public function getSectorDepartmentsName($sectorId){
		$query = $this->db->select('*')->from($this->table_category)->where('id IN (SELECT category_id FROM '.$this->table_sector_department.' WHERE sector_id='.$sectorId.')')->order_by('name','ASC')->get();
		$data = array();
		if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $data[] = $row;
	...
	public function getMultipleSectorDepartmentsName($sectorIds){
		$query = $this->db->select('*')->from($this->table_category)->where('id IN (SELECT category_id FROM '.$this->table_sector_department.' WHERE sector_id IN ('.$sectorIds.'))')->order_by('name','ASC')->get();
		$data = array();
		if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $data[] = $row;
	...
	function getCountTemplateBYLead($leadID){
		$query = $this->db->select('count(*) as countLeadTemplates')->from($this->table_email_template)->where('lead_id',$leadID)->get();
		$data = array();
		if ($query->num_rows() > 0) {  
			$data = $query->first_row();
        }
