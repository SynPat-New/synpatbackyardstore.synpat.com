...
*/?>
<link rel="stylesheet" type="text/css" href="<?php echo $Layout->aws_server_cdn;?>widgets/datepicker-ui/datepicker.css">
...
<script type="text/javascript" src="<?php echo $Layout->aws_server_cdn;?>widgets/datepicker-ui/datepicker.js"></script>
...
#rs tbody tr.main td:first-child,#rs tbody tr.main td:nth-child(3),#rs tbody tr.main td:nth-child(4),#rs tbody tr.main td:nth-child(6){text-align:center;}
span.list_special{margin-right: 15px;cursor: pointer;color: #2d2d2d;font-size: 15px;background: rgb(212,212,212);padding: 2px 5px;margin-bottom: 7px;display: inline-block;border-radius: 11px;}
span.list_special:hover,span.list_special.selected{color:#c20000;text-decoration:underline}
span.list_special:last-child{margin:0;}
.mrg10L{margin-left:10px;}
.page-n,.page-p{margin-left:20px;}
.alpha{text-transform:uppercase;font-size:14px;margin-left:2px;}
.alpha:before{content:" | "}
.alpha:first-child:before{content:""}
.alpha:last-child{margin-right:0}
.alpha.active{color:#c20000;}
#btnCalendar{display:none;}
#ui-datepicker-div{background:#fff;padding:0 8px 0 0;border:1px solid #efefef;border-radius:2px}
.ui-datepicker-trigger{background:none;border:0px;margin-top:10px;cursor:pointer;}
.ui-state-disabled{background:#efefef !important;color:#555a60 !important;}
.ui-state-disabled .ui-state-default{color:#fff;}
.ui-datepicker-prev.ui-state-disabled,.ui-datepicker-next.ui-state-disabled{background:none!important;color:#fff!important;text-indent:-9999px;border:0px !important;cursor:default;display:none;}
</style>
	...
	<div class="col-xs-12" style='width:100%;'>		
		<div class="row btnList" style='margin-bottom:10px;'>
			<div class="col-xs-12">
				...
				<div class="col-xs-2" id="upload_backups">
					<span>Upload backup:</span><input type="input" id="btnCalendar" class="btnCalendar" />
					<?php 
						$todayDate = strtotime('now');
						$yesterday = strtotime('-1 days',$todayDate);
						$dayBeforeYesterday = strtotime('-1 days',$yesterday);
						$dayDayBeforeYesterday = strtotime('-1 days',$dayBeforeYesterday);
					?>
					<!--<a href='javascript://' onclick="uploadBackupCompanyData('<?php echo $yesterday;?>')" class='pull-left mrg5R'><i class='fa fa-database'></i> <?php echo date('M d, y',$yesterday)?></a>
					<a href='javascript://' onclick="uploadBackupCompanyData('<?php echo $dayBeforeYesterday;?>')" class='pull-left mrg5R'><i class='fa fa-database'></i> <?php echo date('M d, y',$dayBeforeYesterday)?></a>
					<a href='javascript://' onclick="uploadBackupCompanyData('<?php echo $dayDayBeforeYesterday;?>')" class='pull-left mrg5R'><i class='fa fa-database'></i> <?php echo date('M d, y',$dayDayBeforeYesterday)?></a>-->
				</div>
			</div>
		</div>
	</div>	
...
function changeActivityType(o){
	...
	if(_leadID==0){
		...
		if(_activity==1){
			window.parent.jQuery("#aquisitionTable").addClass("hide").removeClass("show");
			window.parent.jQuery("#activityTable").addClass("show").removeClass("hide");
			window.parent.displaySaleActivityTable('',jQuery("#salesActivityButton"));
		}else if(_activity==2){
			window.parent.jQuery("#aquisitionTable").addClass("show").removeClass("hide");
			window.parent.jQuery("#activityTable").addClass("hide").removeClass("show");
		...
		if(o.val()>0){
			if(!jQuery('input[name="list_selected"]').is(':checked')){
				jQuery('input[name="list_selected"]').eq(0).trigger("click");
			} else {
				getAllCompanyToLead(jQuery('input[name="list_selected"]').eq(0));
			}
		} else {
...
userColumns = ["Invited","General <input type='checkbox' onclick='checkedMeAll(jQuery(this))'/>","Merging","Survivor","Company","<span class='fa fa-exclamation'></span>","Users","Lin","Size","Industry","Specialties","Sectors","Categories","SubCategories","Technologies","SubTechnologies"];
jQuery(document).ready(function(){
	var width = jQuery(window).width();
	var height = jQuery(window).height();
	jQuery( "#btnCalendar" ).datepicker({
      showOn: "button",buttonText:"<i class='fa fa-calendar'></i>",
        minDate: -15, maxDate: -1,
		dateFormat:'yymmdd',
		onSelect:function(dateText, inst){
			uploadBackupCompanyData(dateText);
		}
    });
	...
	jQuery("#search_specialist").autocomplete({
		minLength: 2,
		source: function (request, response) {
			oldSelectedList.type = "specialties";
			oldSelectedList.list = [];
			oldSelectedList.eq = 10;
			_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,no_of_size:_filterNoOfSize,label:_filterLabel,specialist:_filterSpecialties,start:_startTag,end:_current,sort_field:_sortField,sort_by:_sortBy,t:true,ignore:oldSelectedList.type,tagSearch:request.term}
			jQuery.ajax({
				type:'POST',
				url:__baseUrl+'opportunity/get_filter_list/'+_leadID+'/'+_activity+'/ajax/'+p,
				dataType: "json",
				data: _filters,
				success: function (data) {
					formatSpeciallist(data.data);
					response($.map(data.data, function (item) {
						return {
							label: item[oldSelectedList.type],
							value: item[oldSelectedList.type],
							realValue: item[oldSelectedList.type]
						};
					}));
				},
			});
...
function filterDropDrown(){
	_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,no_of_size:_filterNoOfSize,label:_filterLabel,specialist:_filterSpecialties,start:_startTag,end:1000,sort_field:_sortField,sort_by:_sortBy,t:true,ignore:oldSelectedList.type,tagSearch:''}
	jQuery.ajax({
		url:__baseUrl+'opportunity/get_filter_list/'+_leadID+'/'+_activity+'/ajax/'+p,
		type:"POST",
		data:_filters,
		...
		success:function(list){	
			if(oldSelectedList.type!="specialties"){
				_selectUl = "<ul class='open_list'>";
				if(list.data.length>0){
					for(let i=0;i<list.data.length;i++){
						let l = list.data[i];
						let _checked = "";
						if(oldSelectedList.list.length>0){
							let crrValue = l[oldSelectedList.type];
							if(oldSelectedList.type=="company_size"){
								crrValue = l.type;
							}
							if(!isNaN(parseInt(crrValue)) && oldSelectedList.type!="company_size"){
								crrValue = parseInt(crrValue);
							} 
							if(oldSelectedList.list.indexOf(crrValue)>=0){
								_checked = "CHECKED='CHECKED'";
							}
						}
						let tagName = (l[oldSelectedList.type]!='')?l[oldSelectedList.type]:'None';
						let _value = l[oldSelectedList.type];
						if(oldSelectedList.type=="company_size"){
							_value = l.type;
						}
						_selectUl +="<li style='float:left;width:100%;'><span style='float:left;'><input type='checkbox' value='"+_value+"' "+_checked+" /></span><span>"+tagName+"</span>";
					}
				}
				_selectUl += "</ul>";
				jQuery('.fixedHeader').find('thead th').eq(oldSelectedList.eq).find('.list').empty().html(_selectUl);
			} else {
				totalTags = list.count_all;
				formatSpeciallist(list.data);
			}
		}
	}).always(function(){
		/*getAllSpecialist(false);*/
	});
}
function findMarketSectors(){
	jQuery.ajax({
		...
		if(d.length>0){
			...
			jQuery(".fixedHeader").find('thead th').eq(11).find('.list_options').find('.list').empty();				
...
function findIndustry(){
	jQuery.ajax({
		...
		if(d.length>0){
			...
			jQuery(".fixedHeader").find('thead th').eq(9).find('.list_options').find('.list').empty();				
...
function findTechnologies(){
	jQuery.ajax({
		...
		if(d.length>0){
			...
			jQuery('.fixedHeader').find('thead th').eq(14).find('.list').empty().append(_selectUl);
		}
	}).always(function(){
...
function getCompanyIndustry(){
	_selectUl = jQuery("<ul/>").addClass('open_list');
	var _selectLi = jQuery("<li/>").css({'float':'left','width':'100%'});
	var checkBox = jQuery('<input/>').attr('type','checkbox').attr('value',"");
	var spanY = jQuery("<span/>").css('float','left').append(checkBox);
	var spanX = jQuery('<span/>').append("None");
	_selectLi.append(spanY).append(spanX);
	_selectUl.append(_selectLi);		
...
function openFilterList(o){
	...
	if(parentTh.hasClass('n')){
		...
		_height = o.parent().outerHeight();
		let hH = '320px';
		_width = 400;
		var leftPostion = _position.left;
		if(parentTh.find('span.filter').text()=="Specialties"){
			_width = jQuery(window).width() - 34;
			leftPostion = 2;
			hH = jQuery(window).height() - 150;
			let _innerHH = hH - 40;
			hH = hH+'px';
			parentTh.find(".list_options").find('.p-absolute .list').css({height:_innerHH+'px'});
			/*_diff = _docW - leftPostion;
			if(_diff<1000){
				_d = _width - _diff ;
				leftPostion -=_d;
				leftPostion -=30;
		...
		parentTh.find(".list_options").find('.p-absolute').css({height:hH,top:_height,left:leftPostion,width:_width+'px',background:'#fff',border:'1px solid #dfe8f1'});
		oldSelectedList.type = parentTh.attr('filter-name');
		oldSelectedList.list = [];
		oldSelectedList.eq = -1;
		if(parentTh.find('.list').find('input[type="checkbox"]:checked').length>0){
			parentTh.find('.list').find('input[type="checkbox"]:checked').each(function(){
				let selectedVal = jQuery(this).val();
				if(!isNaN(parseInt(selectedVal)) && oldSelectedList.type!="company_size"){
					
					selectedVal = parseInt(selectedVal);
				}
				oldSelectedList.list.push(selectedVal);
			});
		} else if(parentTh.find('.list').find('span.list_special.selected').length>0){
			parentTh.find('.list').find('span.list_special.selected').each(function(){
				oldSelectedList.list.push(jQuery(this).text());
			});
		}
		if(oldSelectedList.type!=""){
			parentTh.find('.list').empty();
		}		
		switch(oldSelectedList.type){
			case 'no_of_users':
				oldSelectedList.eq = 6;
				filterDropDrown();
			break;
			case 'no_of_employees':
				oldSelectedList.eq = 7;
				filterDropDrown();
			break;
			case 'company_size':
				oldSelectedList.eq = 8;
				filterDropDrown();
			break;
			case 'industry':
				oldSelectedList.eq = 9;
				filterDropDrown();
			break;
			case 'specialties':
				oldSelectedList.eq = 10;
			...
			break;
			case 'sector':
				oldSelectedList.eq = 11;
				filterDropDrown();
			break;
			case 'tech':
				oldSelectedList.eq = 14;
				filterDropDrown();
			break;
		}
	}
}
var oldSelectedList = {type:'',list:[],eq:-1};
function searchFilter(o){
	...
	switch(_filterName){
		...
		break;
		case 'label':
			_filterLabel = parentTH.find('input[type="radio"]:checked').val();
		break;
		case 'linkedin':
		...
		case 'specialties':
			_filterSpecialties = [];
			if(parentTH.find('.list span.selected').length>0){
				parentTH.find('.list span.selected').each(function(){
					_filterSpecialties.push(jQuery(this).text());
				});
			}
		break;
		case 'company_size':
...
function clearFilter(o){
	...
	parentTH.find('span.selected').removeClass('selected');
	parentTH.find(".list_options").find('.p-absolute').parent().hide();
	jQuery("#rs tbody").find("tr").remove();
	_scrollTable = true;
	resetBatch();
...
function showContactsTable(){
	...
	jQuery.each(userColumns,function(i,c){
		...
		if(_class=="filter" || _class=="filter-sort"){
			switch(i){
				...
				break;
				case 5:
					_filterICON +='<div class="list_options"  style="display:none"><div class="p-absolute"><div class="pull-left" style="width:100%"><a class="btn btn-primary pull-left mrg10L mrg5R mrg5T" style="margin-bottom:10px;font-size:15px" onclick="searchFilter(jQuery(this))" href="javascript://">Apply</a><a class="btn btn-default pull-left mrg10L mrg5T" style="margin-bottom:10px;font-size:15px" onclick="clearFilter(jQuery(this))" href="javascript://">Clear</a></div><div style="width:100%;height:270px;overflow:hidden;overflow-y:scroll" class="list"><ul><li><input type="radio" name="list_selected" value="" checked/>All</li><li><input type="radio" name="list_selected" value="0"/>Not Important</li><li><input type="radio" name="list_selected" value="1"/>Important</li></ul></div></div></div>';
					_filterField = "label";
				break;
				case 6:
					_filterICON +='<div class="list_options"  style="display:none"><div class="p-absolute"><div class="pull-left" style="width:100%"><span class="pull-left"><input type="checkbox" onclick="checkMeCheckAll(jQuery(this))"/>All</span><a class="btn btn-primary pull-left mrg10L mrg5R mrg5T" style="margin-bottom:10px;font-size:15px" onclick="searchFilter(jQuery(this))" href="javascript://">Apply</a><a class="btn btn-default pull-left mrg10L mrg5T" style="margin-bottom:10px;font-size:15px" onclick="clearFilter(jQuery(this))" href="javascript://">Clear</a></div><div style="width:100%;height:270px;overflow:hidden;overflow-y:scroll" class="list"></div></div></div>';
				...
				case 10:
					...
					let _charAnchors="";
					for(let s=0;s<_chars.length;s++){
						_charAnchors+="<a class='alpha ' href='javascript://' onclick='searchSpecialistAt(jQuery(this))'>"+_chars.charAt(s).toUpperCase() +"</a>";
					}
					_filterICON +='<div class="list_options"  style="display:none"><div class="p-absolute"><div class="pull-left" style="width:100%"><a class="btn btn-primary pull-left mrg10L mrg5R mrg5T" style="margin-bottom:10px;font-size:15px" onclick="searchFilter(jQuery(this))" href="javascript://">Apply</a><a class="btn btn-default pull-left mrg10L mrg5T" style="margin-bottom:10px;font-size:15px" onclick="clearFilter(jQuery(this))" href="javascript://">Clear</a><span class="paging pull-left" style="margin-top:10px;display:inline-block;width:100px;"></span><span class="pull-left"><input type="text" class="form-control" style="width:200px;height:22px;font-size:15px;" id="search_specialist" name="search_specialist" placeholder="Search Specialities"/></span><span class="pull-left" style="margin-top:10px;">'+_charAnchors+'</span></div><div style="height:100%;overflow:hidden;overflow-y:scroll;padding:0px 5px;" class="list">'+_list+'</div></div></div>';
					_filterField = "specialties";
				break;
				case 11:
					_list = getSectors();
					_filterICON +='<div class="list_options"  style="display:none"><div class="p-absolute"><div class="pull-left" style="width:100%"><span class="pull-left"><input type="checkbox" onclick="checkMeCheckAll(jQuery(this))"/>All</span><a class="btn btn-primary pull-left mrg10L mrg5R mrg5T" style="margin-bottom:10px;font-size:15px" onclick="searchFilter(jQuery(this))" href="javascript://">Apply</a><a class="btn btn-default pull-left mrg10L mrg5T" style="margin-bottom:10px;font-size:15px" onclick="clearFilter(jQuery(this))" href="javascript://">Clear</a></div><div style="width:100%;height:270px;overflow:hidden;overflow-y:scroll" class="list">'+_list+'</div></div></div>';
	...
	jQuery("#noOfRecords").html(formatter.format(__totalRecords));
	jQuery.each(_companiesINString,function(i,u){
		...
		_tableBodyTd = jQuery("<td/>").append(_l);
		_tableBodyTr.append(_tableBodyTd);
		...
		_tableBodyTd = jQuery("<td/>").append(formatter.format(noOfEmployees));
...
function findCompaniesInLead(leadID , t){
	...
	if(jQuery("#profileListType").val()>0){
		/*_leadID = leadID;
		jQuery.ajax({
			type:'POST',
...
_sortBy = "ASC";
var _filterSector=[],_filterType=[],_filterSubType=[],_filterTechnology=[],_filterSubTech=[],_filterUsers=[],_filterLinkedIn = "",_filterCompanyIndustry=[],_filterSpecialties = [],_filterCompanySize=[],_filterNoOfSize=[],_filterLabel="";
...
function loadTableWithNewData(t,start,end){
	...
	if(_requestAlreadySend===false){		
		...
		_requestAlreadySend = true;
		_url = __baseUrl+'opportunity/sales_contact_test/'+_leadID+'/'+_activity+'/ajax/'+p;
		_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,no_of_size:_filterNoOfSize,label:_filterLabel,specialist:_filterSpecialties,start:start,end:end,sort_field:_sortField,sort_by:_sortBy,t:t};
		jQuery.ajax({
			type:"POST",
			url:_url,
			data:_filters,
...
function formatDataAppendInTable(b,t){
	...
	if(_filterTechnology.length>0){
		if(typeof b.sub_tech!="undefined"){
			if(jQuery('.fixedHeader thead th').eq(15).find('.list_options').find('.list').find('ul').length==0){
				jQuery('.fixedHeader thead th').eq(15).find('.list_options').find('.list').empty();
				if(b.sub_tech.length>0){
					...
					jQuery('.fixedHeader thead th').eq(15).find('.list_options').find('.list').append(_selectUl);
				}
			}						
		}
	}
	if(typeof b.companies!="undefined" && b.companies.length>0){
		...
		for(i=0;i<_data.length;i++){
			if(jQuery.inArray(parseInt(_data[i].id),insertID)<0){
				...
				if(_entry===true){	
					...
					//specialistList.push(_data[i].specialties);
				}
			}
		}		
		if(t===false){
		...
		/*formatSpeciallist(specialistList,false);*/
		jQuery("#noOfRecords").html(formatter.format(__totalRecords));
		let _tableBodyTr = "";
		jQuery.each(_companiesINString,function(i,u){
			_checked="";
			...
			if(t===true){
				_tableBodyTr += "<tr id='"+_increment+"' class='"+_addclass+"'>"+_tdList+"</tr>";
			} else {
				_tableBodyTr = "<tr id='"+_increment+"' class='"+_addclass+"'>"+_tdList+"</tr>"+_tableBodyTr;
			}
...
var start=0;
var end=200;
function getAllSpecialist(f,s,e){
	if(f===true){
		start=s;
		end = e;
	}
	...
	oldSelectedList.list = [];
	oldSelectedList.eq = 10;
	_url = __baseUrl+'opportunity/get_all_specialist';
	jQuery.ajax({
		type:"POST",
		url:_url,
		data:{s:start,e:end},
		...
		success:function(b){	
			totalTags = b.total;
			formatSpeciallist(b.list,true);
		}
	});
}
function searchSpecialistAt(o){
	jQuery('.alpha').removeClass('active');
	o.addClass('active');
	_url = __baseUrl+'opportunity/get_all_specialist_by_char';
	jQuery.ajax({
		type:"POST",
		url:_url,
		data:{q:o.text()},
...
function formatSpeciallist(newSpecialist){
	var _listSpan = "";
	if(newSpecialist.length>0){
		for(let j=0;j<newSpecialist.length;j++){
			if(oldSelectedList.type!=""){
				if(newSpecialist[j][oldSelectedList.type]!=""){
					var _tagName = newSpecialist[j][oldSelectedList.type];
					switch(_tagName.substring(0,1)){
						case '#':
						case '&':
						case '@':
						case '*':
						...
						case '>':
						case '<':
						_tagName = _tagName.substr(1).trim();
						break;
					}
					if(_tagName.substring(0,1)=='"'){
						_tagName = _tagName.replace(/['"]+/g, '');
					} else if(_tagName.substring(0,3)=='and'){
						_tagName = _tagName.substr(3).trim();
					}
					_class = "";
					if(oldSelectedList.list.length>0){
						if(oldSelectedList.list.indexOf(_tagName)){
							_class = "selected";
						}
					}					
					_listSpan += "<span class='list_special "+_class+"' onclick='selectSpecialist(jQuery(this))'>"+_tagName+"</span>";
				}
			}												
		}
	}
	...
	if(newSpecialist.length>0){
		_stringPagination ="";
		if(_startTag>0){
			_stringPagination = "<a title='Previous' class='page-p' data-current='"+_current+"' href='javascript://' onclick='previousTags(jQuery(this))'><i class='fa fa-chevron-left'></i></a>";
		}				
		if(_current<totalTags){
			_stringPagination +="<a title='Next' class='page-n' data-current='"+_current+"' href='javascript://' onclick='nextTags(jQuery(this))'><i class='fa fa-chevron-right'></i></a>";
		}
		jQuery('.paging').html(_stringPagination);
	}
	jQuery(".fixedHeader").find('thead th').eq(10).find('.list_options').find('.list').html(_listSpan);
}
var _startTag = 0;
let _current = 1000;
function previousTags(o){
	_current = o.attr('data-current');
	_current = parseInt(_current);
	/*_current -=1000;*/
	_startTag = _current - 1000;
	console.log('start',_startTag)
	if(_startTag<0){
		_startTag = 0;
	}
	_current = _startTag;
...
function sortTables(){
	jQuery('.sort').off('click').on('click',function(){
		...
		/*remove other sorting icon in the table heading*/
		jQuery('#companies_box').find('th').find('i').not($this).removeClass('icon-desc').removeClass('icon-asc').addClass('icon-default');
		/*End*/
			...
			jQuery.ajax({
				...
				success:function(d){
					...
					if(d.update>0){
						...
						parentC.find('td').eq(6).text(d.survivour_data.companyUsers.length);
...
function checkedMe(o){
	...
	jQuery.ajax({
		...
		success:function(d){
			...
			if(window.parent.jQuery("#btnFormCollapse").hasClass('menu-active')){
				/*Lead form data */
				window.parent.callingOrder(0);
			}
		}
	});
}
...
function sendCompanyRequestToScrapper(cID,linkedIn,type,sendRequest){
	var scrapCompany = {company:cID,url:linkedIn,type};
...
function scrapeAllSelectedCompanies(){
	if(jQuery('.fixedHeader').find('th').eq(1).find('input[type="checkbox"]:checked').length>0){
		_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,no_of_size:_filterNoOfSize,label:_filterLabel,specialist:_filterSpecialties,start:0,end:0,sort_field:_sortField,sort_by:_sortBy,lead_id:_leadID,a:_activity,invite:jQuery('input[name="list_selected"]:checked').val()};
		jQuery.ajax({
			type:'POST',
...
function deleteSelectedCompanies(){
	if(jQuery('input[class="general"]:checked').length>0 || _selectedCompanies.length>0){
		if(jQuery('.fixedHeader').find('th').eq(1).find('input[type="checkbox"]:checked').length>0){
			var _totalRecords = jQuery("#noOfRecords").text();
			if(_unselectedCompanies.length>0){
				_totalRecords = _totalRecords - _unselectedCompanies.length;
			}
			conf = confirm("Delete "+formatter.format(_totalRecords)+" companies based on query result?");
		} else {
			conf = confirm("Delete "+formatter.format(_selectedCompanies.length)+" companies?");
		}
		if(conf){
			if(jQuery('.fixedHeader').find('th').eq(1).find('input[type="checkbox"]:checked').length>0){
				_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,no_of_size:_filterNoOfSize,label:_filterLabel,specialist:_filterSpecialties,start:0,end:0,sort_field:_sortField,sort_by:_sortBy,unselected:_unselectedCompanies};
				jQuery.ajax({
					type:'POST',
					...
					_chkD = jQuery('.fixedHeader').find('th').eq(1).find('input[type="checkbox"]')
					_chkD.prop('checked',false);
					_chkD.removeAttr('checked');
				});
			} else {
				contactID = "";
...
function openClassify(){
	if(jQuery('.fixedHeader').find('th').eq(1).find('input[type="checkbox"]:checked').length>0){
		_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,no_of_size:_filterNoOfSize,label:_filterLabel,specialist:_filterSpecialties,start:0,end:0,sort_field:_sortField,sort_by:_sortBy};
		jQuery.ajax({
			type:'POST',
...
function uploadBackupCompanyData(backupDate){
	let cnfrm = confirm("Are your sure?");
	if(cnfrm){
		jQuery.ajax({
			url:__baseUrl+'opportunity/uploadBackupFile',
			type:'POST',
			data:{table:'company',backup_date:backupDate},
			cache:false
