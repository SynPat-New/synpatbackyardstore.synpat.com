...
_sectors = [
	<?php 
		if(count($market_sectors)>0){
			$i=0;
			echo "{'id':'Unclassify','name':'Unclassify'},";
...
function formatDataAppendInTable(b,t){
	...
	if(typeof b.companies!="undefined" && b.companies.length>0){
		...
		jQuery.each(_companiesINString,function(i,u){
			...
			_tdList.push("<td><input type='checkbox' name='general_select_bulk[]' "+_check+" class='general' value='"+u.id+"' data-linkedin='"+u.linkedin_url+"'/> "+_anchor+"</td>");
