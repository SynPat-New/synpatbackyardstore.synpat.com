...
_companiesINString=[<?php
		if(count($contacts)>0){
			...
			foreach($contacts as $contact){				
				...
				$cat = $contact->department_names;
				$subCat = $contact->sub_department_names;
				/*if($contact->id==43738){
					echo "A".$contact->last_name."B<br/>";
					echo "A".$lastName."B<br/>";
				}*/
				$title = $contact->job_title;
				$title = preg_replace("/\n/", "", $title);
			echo "{'id':". $contact->id.",'company_id':". $contact->company_id.",'first_name':'".$firstName."','last_name':'".$lastName. "','name':'".$name."','title':'" . addslashes($title) . "','company_name':'" . addslashes($contact->company_name) . "','email':'" . $contact->email . "','phone':'" . $contact->phone . "','telephone':'" . $contact->telephone . "','company_tel':'" . $contact->company_tel . "','linkedin_url':'" . $contact->linkedin_url . "','sectors':'" . $contact->sectorName . "','categories':'" . $cat . "','sub_category':'".$subCat."','gateway':'" . $contact->gateway . "','selected':'no','system_date':'".$systemDate."','technolgies':'" . $contact->technology_names . "','sub_technolgies':'" . $contact->sub_technology_names . "'}";
...
function findTechnologies(){
	jQuery.ajax({
		...
		if(d.length>0){
			...
			jQuery('.fixedHeader').find('thead th').eq(10).find('.list').empty().append(_selectUl);
		}
	});
}
