...
<script type="text/javascript" src="<?php echo $Layout->aws_server_cdn;?>js-init/moment-with-locales.min.js"></script>
...
foreach($timeline as $key => $val)
{
	...
	$pla = str_replace('/edit?usp=drivesdk','',$pla);
	$pla = str_replace('/view?usp=drivesdk','',$pla);
	$pla = str_replace('/edit','',$pla);
	$pla = str_replace('/view','',$pla);
	if(!empty($acquisition['acquisition']->ppa)){
		$ppa = $acquisition['acquisition']->ppa;
	} else if(count($acordion_text)>0){
		$ppa = $acordion_text->License->purchase_file;
	}
	$ppa = str_replace('/edit?usp=drivesdk','',$ppa);
	$ppa = str_replace('/view?usp=drivesdk','',$ppa);
	$ppa = str_replace('/edit','',$ppa);
	$ppa = str_replace('/view','',$ppa);
	if(!empty($acquisition['acquisition']->rtp)){
		$rtp = $acquisition['acquisition']->rtp;
	} else if(count($acordion_text)>0){
		$rtp = $acordion_text->License->purchase_buy;
	}
	$rtp = str_replace('/edit?usp=drivesdk','',$rtp);
	$rtp = str_replace('/view?usp=drivesdk','',$rtp);
	$rtp = str_replace('/edit','',$rtp);
	$rtp = str_replace('/view','',$rtp);
	if(!empty($acquisition['acquisition']->ppp)){
		$ppp = $acquisition['acquisition']->ppp;
	} else if(count($acordion_text)>0){
		$ppp = $acordion_text->License->program_policies_file;
	}
	$ppp = str_replace('/edit?usp=drivesdk','',$ppp);
	$ppp = str_replace('/view?usp=drivesdk','',$ppp);
	$ppp = str_replace('/edit','',$ppp);
	$ppp = str_replace('/view','',$ppp);
	if(!empty($acquisition['acquisition']->sla)){
		$sla = $acquisition['acquisition']->sla;
	} else if(count($acordion_text)>0){
		$sla = $acordion_text->License->strategic_license_agreement;
	}
	$sla = str_replace('/edit?usp=drivesdk','',$sla);
	$sla = str_replace('/view?usp=drivesdk','',$sla);
	$sla = str_replace('/edit','',$sla);
	$sla = str_replace('/view','',$sla);
...
<script type="text/javascript">/*<![CDATA[*/_PatentList=[];$(function(){$(".bootstrap-datepicker, .input-group.date").datepicker({format:"yyyy-mm-dd"})});function uploadDocketFileFolder(){jQuery('#spinner-loader-docket-file-folder').css('display','inline-block');jQuery.ajax({type:'POST',url:"<?php echo $Layout->baseUrl;?>opportunity/getStoreDIRList",cache:false,success:function(data){jQuery('#spinner-loader-docket-file-folder').css('display','none');}});};function startUpdatePatent(){if(_token==0||_token==""){alert("Server busy, Please refresh our page.")}else{/*jQuery("#spinner-loader-patent").css("display","inline-block");jQuery.ajax({type:"POST",url:"<?php echo $Layout->baseUrl;?>opportunity/startUpdatePatent",data:{token:_token},dataType:'json',cache:false,success:function(_data){if(_data.length>0){_PatentList = _data; runPatentUpdateList(0);}else{jQuery("#spinner-loader-patent").css("display","none");alert("Server busy. Please refresh your page.")}}})*/}}function startInventionDataList(){/*if(_token==0||_token==""){alert("Server busy, Please refresh our page.")}else{jQuery("#spinner-loader-invention").css("display","inline-block");jQuery.ajax({type:"POST",url:"<?php echo $Layout->baseUrl;?>opportunity/startInventionData",data:{token:_token},cache:false,success:function(a){jQuery("#spinner-loader-invention").css("display","none");startUpdateIllustration();_data=jQuery.parseJSON(a);if(_data>0){jQuery("#data_invention").html('<p class="label-after-btn"><span>Invention data updated</span></p>')}else{alert("Server busy. Please refresh your page.")}}})}startUpdateIllustration();*/}function startUpdateIllustration(){/*if(_token==0||_token==""){alert("Server busy, Please refresh our page.")}else{jQuery("#spinner-loader-illustration").css("display","inline-block");jQuery.ajax({type:"POST",url:"<?php echo $Layout->baseUrl;?>opportunity/updateIllustration",data:{token:_token},cache:false,success:function(a){window.parent.runGrabImagesFromScreenShotAndLinks();jQuery("#spinner-loader-illustration").css("display","none");_data=jQuery.parseJSON(a);if(_data>0){jQuery("#data_illustration").html('<p class="label-after-btn"><span>Illustration data updated</span></p>')}else{alert("Server busy. Please refresh your page.")}}})}*/}function approvalList(){if(_token==0||_token==""){alert("Server busy, Please refresh our page.")}else{jQuery("#spinner-loader-cipo").css("display","inline-block");jQuery.ajax({type:"POST",url:"<?php echo $Layout->baseUrl?>opportunity/approvalList",data:{token:_token},cache:false,success:function(a){jQuery("#spinner-loader-cipo").css("display","none");_data=jQuery.parseJSON(a);if(_data.url!=""){jQuery("#assets_approval").html('<p class="label-after-btn is-blink"><i class="glyph-icon icon-caret-right"></i><span>Wait for CIPO approval</span></p>')}else{alert("Server busy. Please refresh your page.")}}})}}function editInviteesData(a){if(__invitees!=undefined){_invitees=__invitees;for(i=0;i<_invitees.length;i++){if(_invitees[i]["contact"].id==a){jQuery("#inviteesCompanyName").val(_invitees[i]["contact"].company_name);jQuery("#inviteesPersonInCharge").val(_invitees[i]["contact"].person_in_charge);jQuery("#inviteesTelephone").val(_invitees[i]["contact"].telephone);jQuery("#inviteesEmail").val(_invitees[i]["contact"].email);jQuery("#inviteesId").val(_invitees[i]["contact"].id);jQuery("#marketSector>option").each(function(){__id=jQuery(this).attr("value");if(_invitees[i]["sector"].length>0){for(j=0;j<_invitees[i]["sector"].length;j++){if(_invitees[i]["sector"][j].id==__id){jQuery(this).attr("selected","selected")}}}});jQuery(".multi-select").multiSelect("refresh");jQuery(".ms-container").append('<i class="glyph-icon icon-exchange"></i>')}}}}function insertDataDocket(){jQuery("#insert_data_docket").removeClass("hide").addClass("display-block")}function checkListContacts(b){var a=[];jQuery("#list_inv_sec").find(".checkbox").find('input[type="checkbox"]').each(function(){if(jQuery(this).prop("checked")){a.push(jQuery(this).val())}});if(a.length>0){__val=b.val();if(__val!=""){jQuery.ajax({type:"POST",url:"<?php echo $Layout->baseUrl?>opportunity/getCheckListContact",data:{token:JSON.stringify(a)},cache:false,success:function(c){_data=jQuery.parseJSON(c);if(_data.length>0){if(typeof(___table)=="object"){___table.destroy()}jQuery("#datatable-contacts").find("tbody").empty();for(i=0;i<_data.length;i++){__name=(_data[i].name!="")?_data[i].name:_data[i].orgTitle;_checked="";if(__selectC.length>0){for(j=0;j<__selectC.length;j++){if(__selectC[j].contact_id==_data[i].id){_checked='CHECKED="CHECKED"'}}}if(jQuery("#selected_contact").find("table").length>0){jQuery("#selected_contact").find("table").find("tbody").find("tr").each(function(){if(jQuery(this).attr("id")==_data[i].id){_checked='CHECKED="CHECKED"'}})}sectors=_data[i].sectors;if(sectors!=""){sectors=sectors.join(",<br/>")}phoneNumber=_data[i].phoneNumber;if(phoneNumber!=""&&phoneNumber!=undefined&&phoneNumber.length>0){phoneNumber=phoneNumber.join(",<br/>")}jQuery("#datatable-contacts>tbody").append('<tr id="'+_data[i].id+'"><td style="width:30px;"><input type="checkbox" '+_checked+' name="invite[contact_id][]" onclick="getDataContact(jQuery(this))" value="'+_data[i].id+'"/></td><td>'+_data[i].name+"</td><td>"+_data[i].orgTitle+"</td><td>"+jQuery.trim(_data[i].orgName)+"</td><td>"+phoneNumber+"</td><td>"+sectors+"</td></tr>")}___table=jQuery("#datatable-contacts").DataTable({searching:true,scrollY:"554px",scrollX:false,scrollCollapse:true,paging:false});console.log("123321");___table.columns.adjust().draw()}else{jQuery("#datatable-contacts").find("tbody").empty()}}})}}else{if(typeof(___table)=="object"){___table.destroy()}jQuery("#datatable-contacts").find("tbody").empty();_data=__invitees;for(i=0;i<_data.length;i++){__name=(_data[i].name!="")?_data[i].name:_data[i].orgTitle;_checked="";if(__selectC.length>0){for(j=0;j<__selectC.length;j++){if(__selectC[j].contact_id==_data[i].id){_checked='CHECKED="CHECKED"'}}}if(jQuery("#selected_contact").find("table").length>0){jQuery("#selected_contact").find("table").find("tbody").find("tr").each(function(){if(jQuery(this).attr("id")==_data[i].id){_checked='CHECKED="CHECKED"'}})}sectors=_data[i].sectors;if(sectors!=""){sectors=sectors.join(",<br/>")}phoneNumber=_data[i].phoneNumber;if(phoneNumber!=""&&phoneNumber!=undefined&&phoneNumber.length>0){phoneNumber=phoneNumber.join(",<br/>")}jQuery("#datatable-contacts>tbody").append('<tr id="'+_data[i].id+'"><td style="width:30px;"><input type="checkbox" '+_checked+' name="invite[contact_id][]" onclick="getDataContact(jQuery(this))" value="'+_data[i].id+'"/></td><td style="width:200px;">'+_data[i].name+'</td><td style="width:200px;">'+_data[i].orgTitle+'</td><td style="width:300px;">'+jQuery.trim(_data[i].orgName)+'</td><td style="width:150px;">'+phoneNumber+'</td><td style="width:200px;">'+sectors+"</td></tr>")}___table=jQuery("#datatable-contacts").DataTable({searching:true,scrollY:"554px",scrollX:false,scrollCollapse:true,paging:false})}}function changeDocketStatus(o){status=o.data('status');jQuery.ajax({type:'POST',url:__baseUrl+'opportunity/change_docket_status',data:{lead_id:leadGlobal,docket_status:status},success:function(resp){obj = JSON.parse(resp);if(obj.success==1){if(obj.status==1){o.text('Offline');}else {o.text('Live');}}o.data('status',obj.status);jQuery('#otherCategory').val(obj.category);}})}function insertEmbedCode(){jQuery("#embedCode").css("display","block")};function btnModeStatus(bID,container){         
	...
	_foundError = 0;
	_foundErrorMessage = "";
	function runPatentUpdateList(t){
		/*jQuery("#spinner-loader-patent").css("display","");
		if(typeof t!="undefined" && _PatentList.length>0){
			if(typeof _PatentList[t]!="undefined"){
				_st = 0;
				if(t==0){
					_st=1;
				}
				jQuery.ajax({
					type:'POST',
					url:__baseUrl+'opportunity/patent_update',
					data:{p:_PatentList[t].name,i:_PatentList[t].id,l:_PatentList[t].lead_id,s:_st},
					success:function(d){
						jQuery("#spinner-loader-patent").css("display","none");
						if(parseInt(d)==0){
							_foundError++;
							_foundErrorMessage += _PatentList[t].name+", ";
						}
						t = t + 1;
						runPatentUpdateList(t);
					},error:function(){
						jQuery("#spinner-loader-patent").css("display","none");
						_foundError++;
						t = t + 1;
				...
				if(_foundError==0){
					...
					alert("Found error in these patents: "+_foundErrorMessage);
					_foundErrorMessage = "";_foundError=0;
					jQuery("#data_patent_update").html('<p class="label-after-btn"><span>Try after sometime</span></p>')
				}	
				startInventionDataList();
			}
		}
		console.log(_PatentList);*/
	}
					...
					<div class="col-xs-12">
						<div class="form-group bigmr">
							...
							<?php if(!empty($lead_data->technical_file)):?>
								<a class='mrg15R pull-left' onclick="window.parent.open_drive_files('https://docs.google.com/spreadsheets/d/<?php echo $lead_data->technical_file;?>')" href="javascript://">Technical DD</a>
							<?php endif; ?>
							<a data-status="<?php echo (int)$acquisition['acquisition']->docket_status?>" href='javascript://' onclick="changeDocketStatus(jQuery(this))">
								<?php if((int)$acquisition['acquisition']->docket_status==0):?>
								Live
								<?php  else:?>
								Offline
								<?php endif;?>
							</a>
							<a class='mrg15R pull-right'  href="https://pixlr.com/editor/" target="_BLANK">Image crop application</a>
						</div>
					</div>
					  	...
					  	<div class="tab-content mrg10T">
					    	<div role="tabpanel" class="tab-pane active" id="docketTab1">
								<?php 
									...
									$personPhone = "";
									$personPicture = "";
									if(isset($acordion_text->License->person_name) && $acordion_text->License->person_name!=""){
										$personName = $acordion_text->License->person_name;
									}
									if(isset($acordion_text->License->person_email) && $acordion_text->License->person_email!=""){
										$personEmail = $acordion_text->License->person_email;
									}
									if(isset($acordion_text->License->person_phone) && $acordion_text->License->person_phone!=""){
										$personPhone = $acordion_text->License->person_phone;
									}
									if(isset($acordion_text->License->person_picture) && $acordion_text->License->person_picture!=""){
										$image =explode('/',$acordion_text->License->person_picture);
										$personPicture = array_pop($image);
									}
								?>
												...
												<select name="docket[person_picture]" id="docketPicture" class="form-control">
													...
													<?php 
														foreach($docs_list['images'] as $doc){
															$checked="";
															if(!empty($personPicture) && $doc->id==$personPicture){
																$checked="selected='selected'";
															}
													?>
													<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
													<?php
														}
													?>
												</select>
								...
								<div class="row">
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketNo_of_non_us_patents" class="control-label">Image Left (350x260):</label>
											<select name="other[image_left]" id="docketImageLeft" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageLeft) && $doc->id==$imageLeft){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketExpectedPrice" class="control-label">Image Middle (350x260):</label>
											<select name="other[image_middle]" id="docketImageMiddle" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageMiddle) && $doc->id==$imageMiddle){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Right (350x260):</label>
											<select name="other[image_right]" id="docketImageRight" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageRight) && $doc->id==$imageRight){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Four (350x260):</label>
											<select name="other[image_four]" id="docketImageFour" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageFour) && $doc->id==$imageFour){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Five (350x260):</label>
											<select name="other[image_five]" id="docketImageFive" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageFive) && $doc->id==$imageFive){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Six (350x260):</label>
											<select name="other[image_six]" id="docketImageSix" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageSix) && $doc->id==$imageSix){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Seven (350x260):</label>
											<select name="other[image_seven]" id="docketImageSeven" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageSeven) && $doc->id==$imageSeven){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Eight (350x260):</label>
											<select name="other[image_eight]" id="docketImageEight" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageEight) && $doc->id==$imageEight){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Nine (350x260):</label>
											<select name="other[image_nine]" id="docketImageNine" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageNine) && $doc->id==$imageNine){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Ten (350x260):</label>
											<select name="other[image_ten]" id="docketImageTen" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageTen) && $doc->id==$imageTen){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Eleven (350x260):</label>
											<select name="other[image_eleven]" id="docketImageEleven" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageEleven) && $doc->id==$imageEleven){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
									...
									<div class="col-xs-4">
										<div class="form-group input-string-group bigmr" style="border: none !important;">
											<label for="marketProspects" class="control-label">Image Twelve (350x260):</label>
											<select name="other[image_twelve]" id="docketImageTwelve" class="form-control">
												<option value="">-- Select File --</option>
												<?php 
													foreach($docs_list['images'] as $doc){
														$checked="";
														if(!empty($imageTwelve) && $doc->id==$imageTwelve){
															$checked="selected='selected'";
														}
												?>
												<option <?php echo $checked?> value="https://www.googledrive.com/host/<?php echo $doc->id?>"><?php echo $doc->title?></option>
					    	...
					    	<div role="tabpanel" class="tab-pane" id="docketTab4">
					    		<div class="row" style="padding-right: 25px;">
									...
									<div class="col-xs-6">
										<div class="row" style="margin-top: 7px;">
											<label for="marketExpectedPrice" class="col-sm-12">Royalty Report:</label>
					    		...
					    		<div class="row">
									<div class="col-xs-6">
										...
										<div class="form-group input-string-group bigmr" style="border: none !important;">
										 ...
										 <div class="form-group input-string-group bigmr">
											<label class="control-label" for="emebedCCCode">License Cost Price (Upfront / # Participants): </label>
									...
									<div class='mrg20T' style="padding-right: 25px;">
										<label for="marketProspects" class="control-label">Only 54 characters allowed.  <span id="remain_character">0</span></label>
			...
			<div class="col-lg-2">
				<div class="col-width" style="width:230px;" id="from_docket">
					<div style=" margin-top:-2px;">
						<div class="clearfix">
							<button type="submit"  class="btn btn-primary btn-block">Update Change in Content</button>
						...
						<div class="clearfix mrg5T">										
							<div class="todo-list-custom button-list" style="width: 230px; padding-left: 0;">
								<div class="row mrg5T">
									<div class="col-sm-12" id="data_patent_update">
										<a href='javascript://' onclick="startUpdatePatent();" class='btn btn-opp-block btn-default mrg5T'>
											<span>Update Change in Patent List</span>
										</a>
								...
								<div class="row mrg5T">
									<div class="col-xs-12">
										<button class="btn btn-opp-block btn-default mrg5T" type="button" onclick="uploadDocketFileFolder();">Update Docket Structure</button>
...
<div class="modal modal-opened-header fade" id="priceModal" tabindex="-1" role="dialog" aria-labelledby="calendarLabel" aria-hidden="true" style='width:100%; margin-top: 0 !important;'>
	<div class="modal-dialog" style='width:100%;'>
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			...
			<div class="modal-footer">
				<button type="button" onclick="saveModalText(8)" class='btn btn-mwidth btn-default' style='margin-right:1px;'>Save</button>
			</div>
		</div>
	</div>
</div>
...
function openModalContent(type){switch(type){case 1:jQuery("#portfolioModal").modal("show");break;case 2:jQuery("#syndicationModal").modal("show");break;case 3:jQuery("#simulatorModal").modal("show");break;case 4:jQuery("#documentsModal").modal("show");break;case 5:jQuery("#scopeModal").modal("show");break;case 6:jQuery("#qualityModal").modal("show");break;case 7:jQuery("#impactModal").modal("show");break;case 8:jQuery("#priceModal").modal("show");break;}}
...
break;case 8:$text=jQuery("#price_tab").code();if($text!=""){jQuery.ajax({type:'POST',url:"<?php echo $Layout->baseUrl?>opportunity/save_accordion_text",data:{lead_id:leadGlobal,price_tab:$text},cache:false,success:function(){jQuery("#priceModal").modal("hide");}})}
break;}}
...
<link rel="stylesheet" media="screen" href="<?php echo $Layout->aws_server_cdn;?>assets/lib/jQuery-contextMenu/jquery.contextMenu.css">
<link rel="stylesheet" media="screen" href="<?php echo $Layout->aws_server_cdn;?>assets/lib/handsontable.full.css">
<script src="<?php echo $Layout->aws_server_cdn;?>assets/lib/handsontable.full.js"></script>
<script src="<?php echo $Layout->aws_server_cdn;?>assets/lib/bootstrap-typeahead.js"></script>
<script src="<?php echo $Layout->aws_server_cdn;?>assets/lib/jquery.autoresize.js"></script>
<script src="<?php echo $Layout->aws_server_cdn;?>assets/lib/jQuery-contextMenu/jquery.contextMenu.js"></script>
<script src="<?php echo $Layout->aws_server_cdn;?>assets/lib/jQuery-contextMenu/jquery.ui.position.js"></script>
