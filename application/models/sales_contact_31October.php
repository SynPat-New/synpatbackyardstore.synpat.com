<?php 
	...
	$linkedInUsers = getAllLinkedinCompanyUsers();
	if(count($company_size_list)==0){
	...
	<?php 
		if(count($company_size)>0){
			...
			foreach($company_size as $company){
				echo "{'id': '".$company->company_size."','company_size':'".$company->company_size."'}";
...
_linkedinUsers = [
	<?php 
		if(count($linkedInUsers)>0){
			$i=0;
			foreach($linkedInUsers as $linkedin){
				echo "{'id': '".$linkedin->linkedinCompanyUsers."','linked_users':'".$linkedin->linkedinCompanyUsers."'}";
				if ($i < count($linkedInUsers) - 1) {
					echo ",";
				}
			}
		}
...
function getLinkedInUserScrape(){
	...
	jQuery.each(_linkedinUsers,function(i,c){
		if(c.id!=21){
			...
			var checkBox = jQuery('<input/>').attr('type','checkbox').attr('value',c.id);
			var spanY = jQuery("<span/>").css('float','left').append(checkBox);
			var spanX = jQuery('<span/>').append(c.linked_users);
			_selectLi.append(spanY).append(spanX);
			_selectUl.append(_selectLi);
		}		
	});
		...
		jQuery.ajax({
			...
			success:function(b){
				...
				if(typeof b.companies!="undefined" && b.companies.length>0){
					...
					_selectUl = jQuery("<ul/>").addClass('open_list');
					jQuery.each(b.company_size_list,function(i,c){
						if(c.id!=21){
							var _selectLi = jQuery("<li/>").css({'float':'left','width':'100%'});
							var checkBox = jQuery('<input/>').attr('type','checkbox').attr('value',c.id);
							var spanY = jQuery("<span/>").css('float','left').append(checkBox);
					...
					_selectUl = jQuery("<ul/>").addClass('open_list');
					jQuery.each(b.linkedin_company_users,function(i,c){
