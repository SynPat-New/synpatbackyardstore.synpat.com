<?php 
	...
	$company_industry = getAllCompanyIndustry();
	$company_size = getAllCompanySize();
?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="addSelectedCompanyToSector()">Add</button>
      </div>
    </div>
...
<div class="row mainT">	
	<div class="col-xs-12" style='width:100%;'>		
		<div class="row btnList" style='margin-bottom:10px;'>
			<div class="col-xs-12">
				<a href='javascript://' onclick="getAllCompanyToLead(1)" class='btn btn-primary pull-left mrg5R'>Selected</a>
...
jQuery(document).ready(function(){
	...
	setTimeout(callLeadData, 500);
	setTimeout(callCountCompanyUsers, 500);
	...
	setTimeout(findTechnologies, 500);
	callNestable()
	checkBoxClickEvent();
});

function callLeadData(){
	if(jQuery("#rs").find('tbody tr').length>0){
		jQuery.ajax({
...
function callCountCompanyUsers(){
	if(jQuery("#rs").find('tbody tr').length>0){
		...
		setTimeout(callCountCompanyUsers, 100);
	}
}
function findTechnologies(){
	jQuery.ajax({
		...
		if(d.length>0){
			...
			jQuery('.fixedHeader').find('thead th').eq(9).find('.list').empty().append(_selectUl);
		}
	});
}
...
function showContactsTable(){
	...
	jQuery.each(_companiesINString,function(i,u){
		...
		_tableBodyTd = jQuery("<td/>").append("<input type='checkbox' name='general_select_bulk[]' "+_check+" class='general' value='"+u.id+"' data-linkedin='"+u.linkedin_url+"'/> "+_anchor);
		_tableBodyTr.append(_tableBodyTd);
...
_sortBy = "ASC";
var _filterSector=[],_filterType=[],_filterSubType=[],_filterTechnology=[],_filterSubTech=[],_filterUsers=[],_filterLinkedIn = "",_filterCompanyIndustry=[],_filterCompanySize=[];
...
function loadTableWithNewData(t,start,end){
	...
	if(_requestAlreadySend===false){
		...
		_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,start:start,end:end,sort_field:_sortField,sort_by:_sortBy};
		jQuery.ajax({
			type:"POST",
			url:_url,
			data:_filters,
			...
			success:function(b){
				...
				if(_filterSector.length>0){
					if(typeof b.cat!="undefined"){
						if(jQuery('.fixedHeader thead th').eq(7).find('.list_options').find('.list').find('ul').length==0){
							
							jQuery('.fixedHeader thead th').eq(7).find('.list_options').find('.list').empty();
							if(b.cat.length>0){
				...
				if(_filterType.length>0){
					if(typeof b.sub_cat!="undefined"){
						if(jQuery('.fixedHeader thead th').eq(8).find('.list_options').find('.list').find('ul').length==0){
							console.log('Ul subcat empty');
							jQuery('.fixedHeader thead th').eq(8).find('.list_options').find('.list').empty();
				...
				if(_filterTechnology.length>0){
					if(typeof b.sub_tech!="undefined"){
						if(jQuery('.fixedHeader thead th').eq(10).find('.list_options').find('.list').find('ul').length==0){
							console.log('Ul subtech empty');
				...
				if(typeof b.companies!="undefined" && b.companies.length>0){
					...
					jQuery.each(_companiesINString,function(i,u){
						...
						_tableBodyTd = jQuery("<td/>").append(u.company_size);
						_tableBodyTr.append(_tableBodyTd);
						_div = jQuery('<div/>').addClass('overwrap').append(u.industry).attr('title',u.industry);
						_tableBodyTd = jQuery("<td/>").append(_div);
						_tableBodyTr.append(_tableBodyTd);
						_div = jQuery('<div/>').addClass('overwrap').append(u.specialist).attr('title',u.specialist);
						_tableBodyTd = jQuery("<td/>").append(_div);
						_tableBodyTr.append(_tableBodyTd);
						_tableBodyTd = jQuery("<td/>").append(u.sectors);
						_tableBodyTr.append(_tableBodyTd);
...
function deleteSelectedCompanies(){
	if(jQuery('input[class="general"]:checked').length>0){
		...
		if(conf){
			if(jQuery('.fixedHeader').find('th').eq(1).find('input[type="checkbox"]:checked')){
				_filters = {search:jQuery('#search_field').val(),s:_filterSector,sc:_filterType,sst:_filterSubType,stt:_filterTechnology,st:_filterSubTech,u:_filterUsers,filterLinkedIn:_filterLinkedIn,industry:_filterCompanyIndustry,company_size:_filterCompanySize,start:0,end:0,sort_field:_sortField,sort_by:_sortBy};
				jQuery.ajax({
					type:'POST',
...
function findSectorsFromCompanies(c){
	jQuery.ajax({
		type:'POST',
		url:__baseUrl+'opportunity/find_companies_to_sectors',
		data:{companies:c},
		dataType:'json'
	}).done(function(d){
		if(d.length>0){
			jQuery.each(d,function(i,s){
			...
			if(d.error==0){
				...
				if(typeof d.data!=null && typeof d.data !='undefined'){
					if(d.data.length>0){
						...
						_companiesList.map(function(c){
							_inputObj = _tableObj.find('input[value="'+_companiesList.company+'"]');
							if(_inputObj.length>0){
								var trObj = _inputObj.parents('tr');
								trObj.find('td').eq(10).text(d.data.sectorName);
								_categoryName = d.data.department_names;
								_categoryName = _categoryName.split(',');
								trObj.find('td').eq(11).html("<div class='overwrap'>"+_categoryName.join('<br/>')+"</div>");
								_subCateName = [];
								if(d.data.sub_department_names!=false){
									_subCateName = d.data.sub_department_names;
								}
								_subCateName = _subCateName.split(',');
								trObj.find('td').eq(12).html("<div class='overwrap'>"+_subCateName.join('<br/>')+"</div>");
								_techName = d.data.technology_names;
								_techName = _techName.split(',');
								trObj.find('td').eq(13).html("<div class='overwrap'>"+_techName.join('<br/>')+"</div>");
								_subTechName = d.data.sub_technology_names;
								_subTechName = _subTechName.split(',');
								trObj.find('td').eq(14).html("<div class='overwrap'>"+_subTechName.join('<br/>')+"</div>");
							}
						});
					}
				}
