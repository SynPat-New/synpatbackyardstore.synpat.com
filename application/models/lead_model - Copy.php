...
INNER JOIN (
 SELECT last_name
	...
	public function getLeadPatentParentData($leadID){
		...
		$data = array();		
		$allPatentQuery = $this->db->select('distinct(sd.patent_number) as patent_number,b_citations,f_citations,expired,application,title,current_assignee,original_assignee,priority,status')->from($this->table_lead_patent_scrap_data.' as sd')->join($this->table_lead_patent.' as lp','lp.patent_number=sd.patent_number')->where('lp.lead_id',$leadID)->get();
	...
	function findAllDisplayPriceData($licenseID,$select){
		$query = $this->db->select($select)->from($this->table_potential_participates)->where('license_id',$licenseID)->where('display',1)->get();
		$data = array();
		if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $data[] = $row;
