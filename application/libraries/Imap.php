...
class Imap {
	...
	public function searchEmails($searchCriteria){
		$result   = imap_search($this->imap, $searchCriteria);
		$emails = array();
		if($result){
    ...
    protected function formatMessage($id, $withbody=true){
        ...
        if($withbody===true) {
			...
			$email['text_body'] = $this->getTextBody($uid);
        }
    ...
    private function getBody($uid) {
	 ...
	 private function getTextBody($uid) {
       $body = $this->get_part($this->imap, $uid, "TEXT/PLAIN");
       $html = false;
       $body = $this->convertToUtf8($body);
       return array( 'body' => $body, 'html' => $html);
