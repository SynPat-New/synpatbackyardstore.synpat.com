...
if ( ! function_exists('buildCompanySectorMenu'))
{
	function buildCompanySectorMenu($companyID=0,$sectorID=0){
		$CI = &get_instance();
		$CI->load->model('opportunity_model');
		$result ="";
		$allSectors = $CI->opportunity_model->getAllMarketSectors();
		...
		if(count($allSectors)>0){
			...
			$result .= "<li class='dd-item nested-list-item' data-order='{$i}' data-id='{$sec->id}' data-type='sector'>
			  ...
			  <div class='nested-list-content'><input type='checkbox' {$checked} name='company[sector][]' onchange='checkMeWithOther(jQuery(this))' value='{$sec->id}'/><div class='listbox'>{$sectorName}</div></div>";
				$categories =  $sec->sub_sector;
				if(count($categories)>0){					
					$result .= "<ol class=\"dd-list\"  data-t=\"sub-sector\">\n";
					
					foreach($categories as $cat){
						$checked = "";
						if(count($companySectors['sub_sector'])>0){
							foreach($companySectors['sub_sector'] as $pref){
								if($pref->sector_id  == $cat->id){
									$checked='CHECKED="CHECKED"';
									break;
								}
							}
						...
						$result .= "<li class='dd-item nested-list-item' data-order='{$c}' data-id='{$cat->id}' data-type='sub-sector'>
						  <div class='dd-handle nested-list-handle'>
							<i class='icon-move'></i>
						  </div>
...
if ( ! function_exists('buildCompanyCategoriesMenu'))
{
	function buildCompanyCategoriesMenu($companyID=0,$sectorID=0){
		$CI = &get_instance();
		$CI->load->model('opportunity_model');
		$CI->load->model('customer_model');
		$result ="";
		...
		$categories =  $CI->opportunity_model->getAllCatgories();
		if(count($categories)>0){					
